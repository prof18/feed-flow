...
- `.scripts/build-reader-bundle.sh` -> Regenerate the desktop reader-mode ES5 bundles (`readability-es5.js`, `turndown-es5.js`); pass `--check` to verify committed bundles without rewriting them
...
The committed desktop reader bundles in `shared/src/jvmMain/resources/` are generated from `tools/reader-parser-bundle/`. Do not hand-edit `readability-es5.js` or `turndown-es5.js`; update the tool package and run `.scripts/build-reader-bundle.sh` instead. `reader-content-parser.js` is hand-written glue and is not regenerated by that script.
