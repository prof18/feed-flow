...
| REG-117 | GReader Provider Forms | `empty` | Android, iOS | Passing | Android and iOS `117-greader-provider-form-validation.yaml` passed via Maestro CLI on 2026-05-26 for provider navigation, required-field disabled states, deterministic FreshRSS auth error, and deterministic FreshRSS auth success. Android also covers filled FreshRSS connect enablement and password reveal |
