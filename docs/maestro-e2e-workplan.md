...
| F019 | Mock account seed state | In progress | REG-116, REG-118 | `sync-linked-mock` can seed a linked account via `account=fresh_rss`, `miniflux`, `bazqux`, `feedbin`, `dropbox`, `google_drive`, or `icloud`; cloud backup/unlink action mocks still need implementation |
...
| REG-118 | Cloud Provider Mock States | `sync-linked-mock` | Android, iOS | In progress | Android and iOS `118-cloud-provider-mock-states.yaml` passed via Maestro on 2026-05-26 for seeded Dropbox and Google Drive linked states; iOS also covers seeded iCloud linked state. Cloud unlink/backup actions still need stable provider-specific mock hooks |
