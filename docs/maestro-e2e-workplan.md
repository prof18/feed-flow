...
| REG-145 | Network Provider Linked States | `sync-linked-mock` | iOS | Passing | iOS `145-network-provider-linked-states.yaml` passed via Maestro CLI on 2026-05-26; covers seeded Miniflux, BazQux, and Feedbin linked screens plus disabled add-account state without live provider auth. Android coverage for the same provider matrix is not added yet because the local emulator hit Maestro instrumentation startup failures during verification. |
...
| Account provider auth and cloud actions | Android, iOS | REG-116 covers one-account constraint; REG-117 covers required-field validation; REG-118 covers seeded Dropbox and iOS iCloud linked screens; REG-138 covers iOS iCloud unlink; REG-142 covers iOS iCloud backup; REG-145 covers iOS seeded Miniflux, BazQux, and Feedbin linked screens | Mocked success/error auth, Google Drive mock state, Android non-FreshRSS network-provider linked matrices, Android/Dropbox cloud unlink, Dropbox/Google Drive backup actions, and real provider sync remain uncovered without live credentials, a healthy Android runner, or existing provider-side mock support. |
