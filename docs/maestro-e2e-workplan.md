...
| F019 | Mock account seed state | In progress | REG-116, REG-118 | `sync-linked-mock` can seed a linked account via `account=fresh_rss`, `miniflux`, `bazqux`, or `feedbin`; cloud provider mock-auth state still needs implementation |
...
| REG-118 | Cloud Provider Mock States | `sync-linked-mock` | Android, iOS | Blocked | Needs provider-specific mock auth for cloud accounts; network account seeds are available, but Dropbox/Google Drive/iCloud restore and unlink flows still need stable mock hooks |
