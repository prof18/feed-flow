...
rm -f "$IOS_FIXTURE_DIR/feedflow-valid-opml-smoke.xml"
rm -f "$IOS_FIXTURE_DIR/feedflow-invalid-opml.xml"
rm -f "$IOS_FIXTURE_DIR/zz-feedflow-invalid-opml.xml"
rm -f "$IOS_FIXTURE_DIR/feedflow-articles-smoke.csv"
cp "$REPO_ROOT/e2e/fixtures/opml/feedflow-valid-opml-smoke.xml" "$IOS_FIXTURE_DIR/"
cp "$REPO_ROOT/e2e/fixtures/opml/zz-feedflow-invalid-opml.xml" "$IOS_FIXTURE_DIR/"
cp "$REPO_ROOT/e2e/fixtures/csv/feedflow-articles-smoke.csv" "$IOS_FIXTURE_DIR/"

while IFS= read -r files_provider_dir; do
  rm -f "$files_provider_dir/feedflow-articles-smoke.csv"
  ...
  rm -f "$files_provider_fixture_dir/feedflow-valid-opml-smoke.xml"
  rm -f "$files_provider_fixture_dir/feedflow-invalid-opml.xml"
  rm -f "$files_provider_fixture_dir/zz-feedflow-invalid-opml.xml"
  rm -f "$files_provider_fixture_dir/feedflow-articles-smoke.csv"
  rm -rf "$files_provider_fixture_dir/zz-opml-errors"
  cp "$REPO_ROOT/e2e/fixtures/opml/feedflow-valid-opml-smoke.xml" "$files_provider_fixture_dir/"
  cp "$REPO_ROOT/e2e/fixtures/opml/zz-feedflow-invalid-opml.xml" "$files_provider_fixture_dir/"
