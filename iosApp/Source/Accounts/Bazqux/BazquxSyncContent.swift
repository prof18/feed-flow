...
struct BazquxSyncContent: View {
    ...
    private func usernameSection() -> some View {
        Section(
            ...
            footer: {
                Text(feedFlowStrings.bazquxUsernameHint)
    ...
    private func passwordSection() -> some View {
        Section(
            ...
            footer: {
                Text(feedFlowStrings.bazquxPasswordHint)
                    .foregroundColor(.gray)
                    .font(.caption)
            }
        )
        ...
        VStack {
            Form {
                ...
                Section {
                    ...
                    case .none:
                        Text(feedFlowStrings.noBazquxSyncYet)
                            .font(.body)
    ...
    private func makeSyncInfoView(state: AccountConnectionUiState.Linked) -> some View {
        Section {
            VStack(alignment: .leading) {
                Text(feedFlowStrings.bazquxAccountConnected)
                    .font(.body)
                ...
                if let syncedState = state.syncState as? AccountSyncUIState.Synced {
                    VStack(alignment: .leading) {
                        if let lastDownloadDate = syncedState.lastDownloadDate {
                            Text(feedFlowStrings.bazquxLastSync(lastDownloadDate))
                                .font(.footnote)
                        }
                    }
