...
struct MinifluxSyncContent: View {
    ...
    private func serverUrlSection() -> some View {
        Section(
            ...
            footer: {
                Text(feedFlowStrings.minifluxUrlHint("https://miniflux.example.com"))
    ...
    private func passwordSection() -> some View {
        Section(
            ...
            footer: {
                Text(feedFlowStrings.minifluxPasswordHint)
                    .foregroundColor(.gray)
                    .font(.caption)
            }
        )
        ...
        VStack {
            Form {
                ...
                Section {
                    ...
                    case .none:
                        Text(feedFlowStrings.noMinifluxSyncYet)
                            .font(.body)
    ...
    private func makeSyncInfoView(state: AccountConnectionUiState.Linked) -> some View {
        Section {
            VStack(alignment: .leading) {
                Text(feedFlowStrings.minifluxAccountConnected)
                    .font(.body)
                ...
                if let syncedState = state.syncState as? AccountSyncUIState.Synced {
                    VStack(alignment: .leading) {
                        if let lastDownloadDate = syncedState.lastDownloadDate {
                            Text(feedFlowStrings.minifluxLastSync(lastDownloadDate))
                                .font(.footnote)
                        }
                    }
