...
class FeedSyncRepository internal constructor(
    ...
    suspend fun addSourceAndCategories(sources: List<FeedSource>, categories: List<FeedSourceCategory>) {
    ...
    suspend fun deleteFeedItems(feedIds: List<FeedItemId>) {
        if (feedSyncAccountRepository.isSyncEnabled()) {
    ...
    suspend fun updateFeedItemReadStatus(feedItemId: FeedItemId, isRead: Boolean) {
        if (feedSyncAccountRepository.isSyncEnabled()) {
    ...
    suspend fun markAllFeedAsRead(allFeedItemIds: List<FeedItemId>) {
        if (feedSyncAccountRepository.isSyncEnabled()) {
